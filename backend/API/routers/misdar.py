...
misdar_router = APIRouter(prefix="/misdars", tags=["Misdars"])
misdar_types_router = APIRouter(prefix="/misdars/types", tags=["Misdar Types"])
...
class Misdars:
    ...
    @misdar_router.post("/search", response_model=List[MisdarOverviewResponse])
    async def get_misdars(self, request: SearchMisdarRequest):
        return await self.service.search(request)

    @misdar_router.post("/start_scan/{misdar_type}")
    ...
    @misdar_router.post("/scan", response_model=ScanResponse)
    ...
    @misdar_router.post("/scan/late", response_model=ScanResponse)
    ...
    @misdar_router.get("/{soldier_id}/{date}/calender", response_model=List[AttendanceDay])
...
@cbv(misdar_types_router)
class MisdarTypesRouter:
    service: IMisdarService = Depends(get_misdar_attendance_service)

    @misdar_types_router.get("/", response_model=List[MisdarTypeSchema])
    async def get_types(self):
        return await self.service.get_types()

    @misdar_types_router.post("/", response_model=MisdarTypeSchema, status_code=status.HTTP_201_CREATED)
    ...
    @misdar_types_router.get("/{misdar_type_id}", response_model=MisdarTypeSchema)
    ...
    @misdar_types_router.put("/{misdar_type_id}", response_model=MisdarTypeSchema)
    ...
    @misdar_types_router.delete("/{misdar_type_id}", status_code=status.HTTP_204_NO_CONTENT)
